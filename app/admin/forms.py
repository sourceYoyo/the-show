...
class postForm(Form,CKEditor):
    ...
    level = SelectField(u"危险等级")
    content = TextAreaField()
    category = SelectField(u"选择分类")
    ckeditor_content = TextAreaField()
    ...
    def __init__(self, content=None):
        ...
        if content:
            ...
            self.slug.data = content.title
            self.content.data = content.md_text
            self.ckeditor_content.data = content.html_text
            self.category.data = content.category.slug
            self.level.data = content.level            
...
class userForm(Form):
    ...
    group = SelectField(u"用户组", choices=choices, default=["reader"],
